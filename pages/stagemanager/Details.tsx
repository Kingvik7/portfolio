...
const project = {
  ...
  highlights: [
    'Based on macOS Ventura Beta version 2',
  ],
  tag1: 'Redesign',
  tag2: 'Figma',
  tag3: 'Concept'
    ...
    <div className="bg-white mx-4 max-w-5xl xl:mx-auto inset-x-0 md:mx-12 lg:mx-24 px-2">
      <div className="pt-8">
        ...
        <div className="mx-auto max-w-2xl  pt-2 pb-16  lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8  lg:pt-2 lg:pb-8">
          ...
          <h3 className="text-sm font-medium text-gray-900 mt-5">TechStack</h3>
          <div className="flex flex-row gap-2 mt-2 w-full">
                    <div className="bg-gray-100 py-1 px-3 rounded-lg text-sm text-gray-500 tag">{project.tag1}</div>
                    <div className="bg-gray-100 py-1 px-3 rounded-lg text-sm text-gray-500 tag">{project.tag2}</div>
                    <div className="bg-gray-100 py-1 px-3 rounded-lg text-sm text-gray-500 tag">{project.tag3}</div>
                  </div>
            <h3 className="text-sm font-medium text-gray-900 mt-5">Links</h3>
            <form className="">
              <button
                type="submit"
                ...
                <ul role="list" className="list-disc space-y-2 pl-4 text-sm">
                  {project.highlights.map((highlight) => (
                    <li key={highlight} className="text-gray-400">
                      <span className="text-gray-500 font-['SentientItalic']">{highlight}</span>
                    </li>
                  ))}
                </ul>
              </div>
